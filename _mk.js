        ...
        window.navigateDay = function(delta) {
            var d = new Date((window._todayStr||'2026-01-01') + 'T00:00:00');
            d.setDate(d.getDate() + delta);
            var ds = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
            if (delta > 0 && ds > window._todayStr) return;
            if (window._loadDate) window._loadDate(ds);
        };
        window.goToDate = function(ds) {
            if (ds > window._todayStr) return;
            if (window._loadDate) window._loadDate(ds);
        };
        ...
        window._loadDate = loadDate; // 暴露给早期兜底函数
